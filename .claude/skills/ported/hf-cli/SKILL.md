---
name: hf-cli
description: 'HuggingFace Hub CLI (`hf` command, replaces deprecated `huggingface-cli`). Use whenever the user mentions hf, huggingface, Hugging Face, or anything in the HF ecosystem. Covers: auth, cache management, models, datasets, spaces, buckets, repos, papers, jobs, collections, dataset queries, space config, webhooks, HF Inference Endpoints. Also for cloud storage (checkpoints, data pipelines, agent traces). Triggers: hf cli, huggingface cli, hf command.'
---
